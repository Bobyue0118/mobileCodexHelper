...
test('isLoopbackHost accepts macOS local browser hostnames', () => {
  ...
  assert.equal(isLoopbackHost('codex-host.tailnet.ts.net'), false);
});
...
test('shouldShowOwnerAdminLauncher requires a user on a loopback host', () => {
  ...
  assert.equal(shouldShowOwnerAdminLauncher({ hostname: 'codex-host.tailnet.ts.net', hasUser: true }), false);
});
